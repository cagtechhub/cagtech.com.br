...
const applySettings = (item: SiteSettings) => {
  ...
  form.packagesIntroTitle = item.packagesIntroTitle
  form.packagesIntroBody = item.packagesIntroBody
  form.initialProjectTitle = item.initialProjectTitle
  form.initialProjectLead = item.initialProjectLead
  form.initialProjectBulletsText = item.initialProjectBullets.join('\n')
}
  ...
  try {
    applySettings(
      await api.updateSettings({
        ...form,
        initialProjectBullets: form.initialProjectBulletsText
          .split('\n')
          .map((line) => line.trim())
          .filter(Boolean),
      }),
    ...
    <form v-else class="surface space-y-4 p-6" @submit.prevent="onSubmit">
      <div class="grid gap-4 sm:grid-cols-2">
        ...
        <label class="block text-sm text-copy-muted sm:col-span-2">
          ...
          <input v-model="form.packagesIntroTitle" class="field-control mt-1" />
        </label>
        <label class="block text-sm text-copy-muted sm:col-span-2">
          Texto da seção Pacotes
          <textarea v-model="form.packagesIntroBody" rows="3" class="field-control mt-1" />
        </label>
        <label class="block text-sm text-copy-muted sm:col-span-2">
          Título do projeto inicial
          <input v-model="form.initialProjectTitle" class="field-control mt-1" />
        </label>
        <label class="block text-sm text-copy-muted sm:col-span-2">
          Texto do projeto inicial
          <textarea v-model="form.initialProjectLead" rows="3" class="field-control mt-1" />
        </label>
        <label class="block text-sm text-copy-muted sm:col-span-2">
          Itens do projeto inicial (um por linha)
          <textarea v-model="form.initialProjectBulletsText" rows="4" class="field-control mt-1" />
        </label>
      </div>
