...
import type { AdminDashboardStats } from '@cagtech/shared'
...
const cards = computed(() => [
  { label: 'Leads (7 dias)', value: stats.value.contactsNew },
  { label: 'Leads totais', value: stats.value.contactsTotal },
  { label: 'Pacotes', value: stats.value.packagesCount },
  { label: 'Projetos', value: stats.value.projectsCount },
...
const loadDashboard = async () => {
  ...
  try {
    ...
    if (statusCode === 403) {
      error.value =
        'Usuário sem permissão de admin. Inclua o e-mail em ADMIN_ALLOWED_EMAILS no .env.'
...
<template>
  <div class="space-y-8">
    <div>
      ...
      <p class="mt-1 text-sm text-copy-muted">Leads, pacotes e projetos publicados na landing.</p>
    </div>
    ...
    <div v-if="!loading && !error" class="flex flex-wrap gap-3">
      <NuxtLink to="/admin/leads" class="brand-button">Kanban de leads</NuxtLink>
